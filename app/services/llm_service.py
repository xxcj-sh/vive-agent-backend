...
class LLMService:
    ...
    async def _handle_simple_chat_non_stream(
        ...
        if is_anonymous:
            ...
            system_prompts = {
                "general": "参考以下要求：1. 请自然地和用户对话，回复内容不宜太长，参考你的人物设定，长短相接地回复用户; 2. 遵循道德底线，提供情绪价值；3. 尝试分析对方的需求和利用价值，在对话中寻找合作机会",
                "topic": "请帮助用户深入讨论话题.",
            }
    ...
    async def generate_chat_summary(
        ...
        try:
            ...
            # 构建总结提示词
            prompt = f"""总结用户想要做什么，以及聊了什么，忽略 AI 的回复内容，不超过200字。用户和 AI 的聊天记录：{chat_context}"""            
            llm_request = LLMRequest(
                user_id=user_id,
